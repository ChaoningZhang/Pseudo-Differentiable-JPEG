...
class XVID(nn.Module):
    def __init__(self):
        super(XVID, self).__init__()
        self.key = generate_random_key()

    def forward(self, noised_and_cover):
        video_folder_path = "./xvid/" + self.key
        if not os.path.exists(video_folder_path):
            os.makedirs(video_folder_path)
        fourcc = cv2.VideoWriter_fourcc(*'XVID') 
        video = cv2.VideoWriter(video_folder_path + '/video.avi', fourcc, 30, (128, 128))
        ...
        video_saved = cv2.VideoCapture(video_folder_path + '/video.avi')
