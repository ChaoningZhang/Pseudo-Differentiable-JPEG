...
class JpegCompression2000(nn.Module):
    def __init__(self, quality=500):
        super(JpegCompression2000, self).__init__()
        self.quality = quality
        self.key = generate_random_key()

    def forward(self, noised_and_cover):
        jpeg_folder_path = "./jpeg2000_" + str(self.quality) + "/" + self.key
        ...
        for i in range(N):
            ...
            folder_imgs = jpeg_folder_path + "/jpg2_" + str(i).zfill(2) + ".jp2"
            cv2.imwrite(folder_imgs, img, [int(cv2.IMWRITE_JPEG2000_COMPRESSION_X1000), self.quality])
