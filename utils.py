...
from oops_dataset import OOPSDataset
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    try:
        if not train_options.video_dataset:
            ...
            train_images = OOPSDataset(train_options.train_folder, data_transforms['train'])
            train_loader = train_images
            # train_loader = torch.utils.data.DataLoader(train_images, batch_size=train_options.batch_size, shuffle=True,
            #                                         num_workers=4)

            validation_images = OOPSDataset(train_options.validation_folder, data_transforms['test'])
            validation_loader = validation_images
            # validation_loader = torch.utils.data.DataLoader(validation_images, batch_size=train_options.batch_size,
            #                                                 shuffle=False, num_workers=4)
