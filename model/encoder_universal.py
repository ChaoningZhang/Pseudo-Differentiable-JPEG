...
class Encoder(nn.Module):
    ...
    def __init__(self, config: HiDDenConfiguration):
        ...
        layers = [ConvBNRelu(30, self.conv_channels)]
    ...
    def forward(self, image, message):
        ...
        if self.cover_dependent:
            ...
            encoded_image = self.conv_layers(message)
            # concatenate expanded message and image
            concat = expanded_message # torch.cat([expanded_message, encoded_image, image], dim=1)
            im_w = self.after_concat_layer(concat)
